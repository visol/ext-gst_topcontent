...
class tx_gsttopcontent_pi1 extends tslib_pibase {
	...
	function main($content, $conf)	{
		...
		$this->config['startPid'] = t3lib_div::intExplode(',', trim($this->conf['startPid']));
		$this->config['excludePids'] = t3lib_div::intExplode(',', trim($this->conf['excludePids']));
		$this->config['excludePidsR'] = t3lib_div::intExplode(',', trim($this->conf['excludePidsR']));
		$this->config['maxRecs'] = t3lib_div::intInRange($conf['maxRecs'], 0, 200);
		$this->config['ppRecs'] = t3lib_div::intInRange($conf['ppRecs'], 0, 200);
		$this->config['offsetRecs'] = t3lib_div::intInRange($conf['offsetRecs'], 0, 200);
		$this->config['allowedDokTypes'] = t3lib_div::intExplode(',', trim($this->conf['allowedDokTypes']));
		$this->config['maxTextLength'] = t3lib_div::intInRange($conf['maxTextLength'], 0, 500);
		...
		$this->config['linkPageTitle'] = t3lib_div::intInRange($conf['linkPageTitle'], 0, 1);
		...
		$this->config['useTeaserAndBodytext'] = t3lib_div::intInRange($conf['useTeaserAndBodytext'], 0, 1);
		$this->config['onlyOnePerPage'] = t3lib_div::intInRange($conf['onlyOnePerPage'], 0, 1);
...
// ### For dubug purposes only ###: debug('No template code found!');
		...
		$pluginstartingpoints = t3lib_div::intExplode(',', $this->cObj->data['pages']);
		...
		while(list($k, $v) = each($this->config['excludePidsR'])) {
			if($v > 0) {
				...
				// Merge the result with the other pages array. (Note: t3lib_div::array_merge is not used here, because it loses values.)
...
// ### For dubug purposes only ###: debug($in_pid_list);
		...
		$syslanguage = t3lib_div::intInRange($GLOBALS['TSFE']->sys_language_uid, 0);
...
// ### For dubug purposes only ###: return $this->pi_wrapInBaseClass($query);
	...
	$rows = count($resultdata);
		...
		if($this->config['ppRecs'] > 0 && $rows > 0) {
			...
			$page_goto = t3lib_div::intInRange(t3lib_div::GPvar($ppGoto), 1);
...
//if($rows) {
	...
	function GetAllPages($pid_start)	{
		...
		for($i = 0; $i < count($pids_check); $i++) {
			while(list($k, $v) = each($pids_check)) {
				// Recursive call for each subpage. (Note: t3lib_div::array_merge is not used here because it breaks the recursive function call.)
	...
	function GetPageDependendData(&$ref_res, $only_one_perpage)	{
		...
		if($only_one_perpage > 0) {
			...
			while($row = mysql_fetch_array($ref_res))  {
	...
	function RenderLink($pid, $uid = 0, $target = '')	{
		...
		if(strlen($this->config['showRecImg']) > 4) {
			if($this->config['showRecImg'] == $this->deflinkIcon) {
				$linkpath = t3lib_extMgm::siteRelPath($this->extKey).'res/';
			} // end if
...
if (defined("TYPO3_MODE") && $TYPO3_CONF_VARS[TYPO3_MODE]["XCLASS"]["ext/gst_topcontent/pi1/class.tx_gsttopcontent_pi1.php"]) {
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]["XCLASS"]["ext/gst_topcontent/pi1/class.tx_gsttopcontent_pi1.php"]);
} // end if
